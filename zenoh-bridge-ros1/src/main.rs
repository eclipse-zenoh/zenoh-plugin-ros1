...
async fn main() {
    ...
    if config.plugin("rest").is_some() {
        plugins_mgr = plugins_mgr.declare_static_plugin::<zenoh_plugin_rest::RestPlugin, &str>("ros1", true);
    }

    // declare ROS2DDS plugin
    plugins_mgr = plugins_mgr.declare_static_plugin::<zenoh_plugin_ros1::Ros1Plugin, &str>("ros1", true);
