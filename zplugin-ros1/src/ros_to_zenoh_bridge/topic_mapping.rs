...
impl Ros1TopicMapping {
    pub fn topic_mapping(ros1_client: &ros1_client::Ros1Client) -> rosrust::api::error::Response<Ros1TopicMapping> {
...
// PRIVATE:
    fn new(state: &rosrust::api::SystemState, 
           topics: &Vec<rosrust::api::Topic>) -> Ros1TopicMapping {
    ...
    fn fill(dst: &mut HashSet<rosrust::api::Topic>, 
        ...
        for item in data.iter() {
            ...
            if topic.is_none() {
                ...
                dst.insert(rosrust::api::Topic{name: item.name.clone(), datatype: "*".to_string()});
            }
            else {
                let t = topic.unwrap();
                dst.insert(t.clone());
